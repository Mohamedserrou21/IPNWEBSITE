...
if (isset($_GET['id'])) {
	...
	if ($result->num_rows > 0) {
		...
		while ($row = $result->fetch_assoc()) {
			 ...
			 $name = mysqli_real_escape_string( $_row['name']);
    $email = mysqli_real_escape_string( $_row['email']);
    $password = mysqli_real_escape_string( $_row['password']);
			$id = $row['id'];
		}
...
<h2>Article Update Form</h2>
		...
		<form action="up article.php" method="post" enctype="multipart/form-data">
		  <fieldset>
		   ...
		   name:<br>
		    ...
		    <br>
			    CONTEXT:<br>
		    <textarea  name="email"  class="input"> <?php echo $email; ?></textarea>
